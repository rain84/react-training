...
import { Box, ButtonPlay, Input } from '@/ui'
...
export const ReusableComponents = () => {
	...
	const tweens = {
		1: useRef<gsap.core.Tween | null>(null),
		3: useRef<gsap.core.Tween | null>(null),
	}

	//  update with "useDeferredValue"
	const [width, setWidth] = useState(INITIAL_WIDTH)
	const updateWidth: ChangeEventHandler<HTMLInputElement> = useDebounce(
		(e: React.ChangeEvent<HTMLInputElement>) => setWidth(+e.target.value),
		INPUT_DELAY,
	)
	...
	useGsap(
		(gsap) => {
			...
			tweens[1].current = gsap.to(box[1].current, {
				rotation: 360,
				repeat: -1,
				repeatDelay: 1,
				yoyo: true,
			})
			tweens[3].current = gsap.to(box[3].current, {
				x: width,
				repeat: -1,
				repeatDelay: 1,
				yoyo: true,
	...
	return (
		<div className="flex flex-col justify-between space-y-4">
			<ButtonPlay onClick={toggle} className="text-center" isPlaying={isPlaying} />

			<Input.Number defaultValue={width} onChange={updateWidth} className="w-20" />

			<Box ref={box[1]} className="bg-green-600" children="Box" />
			<Box ref={box[2]} className="bg-green-600">
				<span className="inline-block font-bold scale-50">Don't Animate Me</span>
			</Box>
