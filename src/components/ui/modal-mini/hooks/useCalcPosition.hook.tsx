...
export const useCalcPosition = (
	...
	useEffect(() => {
		...
		modalRef.current.style.top = `${parseInt(style.container.height) + offset + parseInt(border)}px`
		...
		const isOuterOfTheBottom =
			containerRect.y + containerRect.height + offset + height + parseInt(border) >
			parseInt(style.body.height)

		const canBeCentered = containerRect.x + containerRect.width + diff < parseInt(style.body.width)
		...
		if (isOuterOfTheBottom) {
			modalRef.current.style.bottom = `-${containerRect.y - offset - parseInt(border)}px`
		}
