...
import { Error, Items, Loading } from './items.component'
import type { OnChangeEvent, SearchBoxProps, TItemsState } from './search-box.types'
...
export const SearchBox = ({
	...
	const [itemsState, setItemsState] = useState<TItemsState>(isAsync ? 'loading' : 'ready')
	...
	const onChange = useDebounce((e: OnChangeEvent) => {
	...
	useEffect(() => {
		...
		const canProcess =
			...
			(!isAsync && items_ instanceof Promise) || (!isAsync && itemsState === 'ready')
	...
	return (
		<section className="m-5 inline-block">
			...
			<Switch
				loading={Loading}
				ready={<Items items={filteredItems} visible={!!query.length} query={query} index={index} />}
				error={Error}
			>
				{itemsState}
			</Switch>
